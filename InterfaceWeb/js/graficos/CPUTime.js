...
$(document).ready(function () {
  ...
  var test = 'http://localhost:8080/cpu'
  $.getJSON(test, function (json) {

    for (i = 0; i < json.rows.length; i++) {
      
      x.push(json.rows[i].timestamp)
      y.push(json.rows[i].usage)

      var ts = new Date(json.rows[i].timestamp)
      tse.push(ts.toLocaleTimeString())
    }

    // Area Chart Example
    var chart = document.getElementById("CPUTime");
